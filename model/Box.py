...
class Box:
    ...
    def __init__(self, game_manager, x, y, app, map_controller):
        ...
        self.x = x
        self.y = y
        self.map_contoller = map_controller
        self.map_contoller.block_field(self.y, self.x, self)
        self.box_gfx = BoxGFX(game_manager, x, y, app, map_controller)

    def can_fall(self):
        return self.map_contoller.check_field(self.y - 1, self.x)

    def refresh(self):
        self.box_gfx.refresh()

    def move_up(self):
        if self.map_contoller.check_field(self.y, self.x) == self:
            self.map_contoller.release_field(self.y, self.x)
        self.y = self.y + 1
        self.map_contoller.block_field(self.y, self.x, self)
    ...
    def can_move_horizontal(self, direction):
        return self.map_contoller.check_field(self.y, self.x + direction)
